...
execute if entity @e[tag=EM_shield,limit=1] as @e[tag=EM_shield] at @s unless score @s EM_shield_time matches 1.. positioned ^ ^ ^0.8 positioned ~-1 ~-0.5 ~-1 if entity @e[type=trident,dx=1,dy=1.5,dz=1,nbt={inGround:0b}] if entity @e[type=trident,dx=1,dy=1.5,dz=1,nbt={inGround:0b}] at @s run function elite_mobs:skills/shield/reflect3
