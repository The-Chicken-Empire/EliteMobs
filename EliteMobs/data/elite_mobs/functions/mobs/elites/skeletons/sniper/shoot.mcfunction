...
execute run data modify entity @e[tag=EM_target,limit=1,sort=nearest] Owner set from entity @s UUID

execute anchored eyes positioned ^ ^ ^ run summon area_effect_cloud ~ ~ ~ {Tags:["EM_sniper_shoot"]}
execute as @e[tag=EM_sniper_shoot,limit=1,sort=nearest] at @s facing entity @a[tag=EM_sniper_target,limit=1] eyes positioned ^ ^ ^ positioned 0.0 0.0 0.0 run tp ^ ^ ^5
